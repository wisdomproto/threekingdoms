...
## 1. 문제
전투 HUD는 데스크톱 전용이다: **브레이크포인트 분기가 없다**(`viewport.width`는 배치 클램프/좌우 플립에만 쓰인다; `Formation`의 `useNarrow`는 편성 화면). <768px에서 좌 컬럼(`min(64vw,280px)`)과 우측 스택(미니맵 144 + 컨트롤 104px)이 맵을 덮고, 행동 메뉴(`ActionMenu`, 폭 96·버튼 34px)는 손가락보다 작으며, 턴종료(우하단 56px)·AudioControl(전역 좌하단 40px)·대사창이 하단을 나눠 갖는다.
...
- **패널 상태(순수)**: `bottomPanelState(ui)` → `"hidden"`(battleOver — ResultSequence 오버레이에 z-index가 없어 패널이 결산을 덮는다) / `"collapsed"`(idle·enemyTurn·autoTurn·animating·confirmEndTurn) / `"expanded"`(selected·postMoveMenu·targetSelect·confirmAttack·strategyMenu·strategyTarget·itemMenu·itemTarget). idle에 `inspectedId`가 있으면 collapsed 바에 그 유닛 한 줄(탭하면 상세 시트) — 모바일에선 부유 `InspectPopup`을 렌더하지 않는다(같은 정보 2중 표시 방지). 사용자 토글 없음(YAGNI — 상태기계가 곧 상태). `previewWalking` 중엔 행동 버튼 비움(ActionMenu와 같은 규칙).
- **턴종료는 패널 안으로**: 모바일에선 `TurnBanner`의 절대좌표 턴종료를 숨기고(`hideEndTurn` prop) collapsed 바 우측에 같은 이벤트(`endTurnPressed`)를 52px 버튼으로. 규칙 `canEndTurn(ui, vm)`(= idle · player 페이즈 · ongoing)은 TurnBanner에서 export해 공유.
- **AudioControl 충돌 = CSS 변수 한 줄**: `BottomPanel` 자신이 ResizeObserver 효과로 `--tk-bottom-inset`(패널 높이)을 `documentElement`에 쓰고 언마운트 시 `removeProperty`(데스크톱 전환 = 언마운트라 자동 정리); `AudioController`의 `bottom`이 `calc(max(10px, env(safe-area-inset-bottom)) + var(--tk-bottom-inset, 0px))`. 패널엔 스페이서 없음(항상 들어올림).
...
│ collapsed: [유닛 한 줄(조회 중일 때)]              [턴 종료 52px] │  (AudioControl은 CSS 변수로 패널 위로 올라감)
...
- 버튼: `minHeight:52`, `fontSize:16`, 4열 grid(`repeat(4, 1fr)`), `gap:6`, 그리드 `maxHeight:50vh; overflowY:auto`(책략/도구 항목 수는 데이터 의존). `disabled`는 기존 dim 규칙. `accent` 색은 기존 `Item.accent`.
- 유닛 행 데이터 = `UnitVM`(`name/className/level/troops/maxTroops/sp/maxSp/side/acted`) — 활성 유닛 id는 `UnitPanel.activeUnitId` 규칙(export; **itemMenu/itemTarget 누락 버그 동시 수정**) — `confirmAttack`은 `prior.unitId`. 초상은 `UnitPanel.PortraitBox` 재사용(onError 레이스 방어 내장).
- 패널 높이는 `ResizeObserver`로 측정해 `--tk-bottom-inset`에 반영(collapsed ≈ 56, expanded ≈ 200~320). `hudMode(0)`(미측정 첫 프레임)은 desktop이지만 부트 장막(z60)이 가리므로 무시.
...
## 5. 이벤트 흐름
- 행동 버튼 = `itemsFor`의 `onPress`(이미 `dispatch` 클로저). 턴종료 = `dispatch({type:"endTurnPressed"})`(TurnBanner와 동일). 취소 = `Item`에 이미 포함(postMoveMenu는 `menuCancel`, 대상 선택 상태는 `cancel`).
