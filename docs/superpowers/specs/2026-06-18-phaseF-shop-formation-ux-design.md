...
>
> ⚠️ **2026-07-03 편성 화면 v2로 대체됨** — 편성 UI는 레퍼런스 3존(카드 그리드 / `CommanderDetail` / `SortieBar`)으로 전면 재구성됐고, 장비는 슬롯제(무기/말/보물), 소모품은 부대 공유 창고(장착 폐지)로 전환. 현재 SSOT = CLAUDE.md §10 + 메모리 consumable-shared-pool. 이 문서는 그 이전 폴리시 설계 기록.
