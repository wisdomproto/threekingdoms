...
| `apps/web/src/battle/BattleScreen.tsx` (수정) | `makeCtx()`가 `sandbox: boolean`(= `__lab` 분기 여부)을 반환; 476~477행 `stageId`/`sandbox` prop과 529행 종료 결정을 `ctx.stage.id === LAB_STAGE_ID` 대신 이 플래그로. sandbox면 `<PauseMenu … sandbox={sandbox} />` |
