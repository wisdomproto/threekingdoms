...
## 8. 구현 후 아트디렉션 개선 (2026-06-17, 유비 생성 검수 반영)

- **등급 격상 강화**: §4.1의 "색/장식만"을 확대 — 등급↑ 시 갑옷뿐 아니라 **탈것 호화도도 격상**. 전차=말 1→2→3마리 + 금장·용 조각·깃발·보석 바퀴 / 기병=마갑·깃 격상 / 사륜거=장식 격상. 형태·무기·얼굴은 여전히 불변(`POSE_MOUNT` 클래스별 행 격상 문구).
- **시그니처 무기 명시 주입**: S-pose는 이름-only라 모델이 무기를 오작화(유비→창) → `SIG_BY_NAME`(★/주요 17명, 기존 B-5/F-5 sig 재사용) 주입으로 무기·정체성 고정. 군소 장수는 일반 무기(주입 안 함).
- **행별 일관성 제약**: 같은 등급 행의 3포즈는 말 수가 동일해야 함(전차 "말 수 행별 고정 — 윗행1/가운데2/아랫행3, 칸끼리 불변" 명시).

## 9. 방향(facing) 보정 (2026-06-21, 관우 생성 검수 반영)

- **SD facing = screen-left 고정**(CLAUDE.md §4/§7 — 코드 미러링 기준). 게임은 왼쪽 바라보기를 기준으로 두고 우향 이동 시 코드로 좌우 반전. 한 캐릭터만 오른쪽이면 인게임 방향이 전부 거꾸로.
- **모델이 좌/우를 자주 반전**: S-pose 프롬프트에 "every cell ... FACING SCREEN-LEFT" 명시돼 있어도 절반쯤은 오른쪽으로 나온다(유비=왼쪽 OK, 관우=오른쪽 반전). 이미지 생성 모델의 고질적 약점 — 프롬프트 강화로는 신뢰 불가.
- **해결 = 컷 단계 보정**(프롬프트와 싸우지 않음): `cut_posesheet.py --flip` — 각 셀을 좌우 미러. **셀 위치로 idle/move/attack을 판정**하므로 칸 안 그림만 뒤집히고 포즈 매핑은 불변. 그림 품질이 좋으면 재생성 없이 방향만 교정.
- **방향 판정 = 비전 QA**(CLAUDE.md §4): 📤 시트 넘기기 후 Claude가 `_posesheet.png`를 Read로 보고 screen-left/right를 판정해 `--flip` 여부 결정.
