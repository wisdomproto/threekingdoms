...
모듈은 serve.py(:8081) 아래에서 같은 출처로 로드된다. `file://`로 열면 module import가 막히므로 안내 문구 1줄.
...
EVENT: id trigger outcome once            (type 은 새 원소 생성 시에만 'duel')
REINF: id side units trigger once         (once 는 원본 값 유지, 새 원소만 true)
...
- 원본에 그 키가 있었으면 항상 쓴다(빈 배열이어도).
- 원본에 없었으면 값이 있을 때만 쓴다(빈 배열·`undefined`·`null`은 생략).
- `camera`·`reward`·`levelCap`을 UI에서 해제하면 키를 **삭제**한다(명시적 `delete`).
...
| **Publish** | 「Publish 검사」 버튼 | `POST /validate-data` → serve.py가 `pnpm --filter @tk/data test` 실행 (`index.ts`가 27 스테이지·30 맵 전부 zod `safeParse`, 파일명 붙은 에러) | 패널에 통과/실패 + 출력 꼬리 60줄 |

Publish 검사는 **디스크의 저장본**을 검사한다 — 버튼 옆 "저장 후 검사" 문구. 새 의존성 없음.

## 6. 테스트 (`packages/data/test/editor-roundtrip.test.ts`)
1. **round-trip 동일성**: `stages/*.json`·`maps/*.json` 각각 `JSON.stringify(serialize(load(x))) === JSON.stringify(x)` — 구조 + 키 순서. (원문 텍스트 비교는 하지 않는다: 맵 9개가 2-space 규격과 다르다.)
2. **편집 의미론**: 유닛 `x` 변경 → 그 필드만 바뀌고 `scenario`·유닛 미지 필드 보존 / `camera=null` → 키 삭제 / 새 이벤트 → `type:'duel'`·`once:true` / 유닛 삭제 → 원소 제거 / `turnLimit` 없는 입력 → 저장에도 없음.
3. 기존 `pnpm test` 게이트(data 패키지)에 포함.

## 7. 에러 처리
- 스키마상 유효하지 않은 원본도 로드된다(보존이 목적). 유효성은 Publish 단계가 말한다.
- `/validate-data`: 타임아웃 120초, 동시 실행 1개(락, 진행 중이면 409), 출력 마지막 60줄만. 실패도 200 + `ok:false`.
