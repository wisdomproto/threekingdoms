...
1. ✅ **Object-sheet cut tool + asset generation** — DONE 2026-06-21/22 (`cut_object_sheet.py` gap-detect cut → `/assets/objects/*` 41 sprites; K-4 walls/gates render verified in game).
2. **Gate/wall destructible state events** — wire engine events (관문돌파 §5, 화공) → `ObjectLayer.setObjectState(gx,gy,'open'|'destroyed')` + terrain passability change + K-8 dust/smoke VFX. Renderer half (`setObjectState`) is already built here; the engine trigger/event schema is the open design item. **← the only Chunk 3 item still open.**
3. ✅ **`decorations` data field** — DONE 2026-06-30 (main e86c1cf): `DecorationSchema {cell, kind, flip?, scale?}` on stage JSON, `kind` = z.enum whitelist of 12 ground-level flavor props (blocking-looking objects rejected at load — visual honesty on walkable cells); ObjectLayer renders per chunk on top of terrain auto-deco. 113 props authored for stages 01-09 (faction banners, fire-attack braziers, siege debris, Dong Zhuo's abandoned carts, reeds), machine-validated placement (bounds / plain·grass·waste only / no unit-spawn collision / no dupes) + contract test `packages/data/test/decorations.test.ts` enforcing the same rules for future stages. Stages 10-27 to be authored together with their painted backgrounds.
