...
-   ✅ Story Editor **v1** (2026-09-12, VN 씬·전투 중 대사 편집 + 씬 미리보기 왕복)
-   ✅ Story Editor **v2** (2026-09-12, 모션코믹 `ComicScene` = 페이지+칸+카메라: 스키마·`ComicScenePlayer`·썸네일 위 칸 드래그 편집기·검증·매니페스트·보드 카드·E2E `e2e:comic`; `docs/superpowers/specs/2026-09-12-story-editor-v2-comic-design.md`)
...
**미결(길중 결정 필요)**: ① 공식 콘텐츠 2탄(진시황 / TROIA / DAVID) — *2026-09-19 검토 근거(결정 아님)*: 언어권별 안 = 한·중 삼국지 → 초한(패왕전이 조클 역대 좋아요 2위) / 영어권 트로이(파이어엠블렘 문법, 본편 시장 북미 중심) — `docs/reference/fire-emblem-research-2026-09.md`, `docs/reference/jojo-club-community-research-2026-09.md` ② ~~Story Editor 착수 시점~~ → **v1(현행 VN·대사 편집)은 P2에서 착수·완료(2026-09-12)**; ~~남은 결정 = 모션코믹 **포맷**(타임라인 데이터 계약)~~ → **결정(2026-09-12): 포맷 = 페이지 + 칸(panel) + 카메라(`ComicScene`)** — 페이지 지면 이미지 여러 장 + 페이지마다 칸 정규화 사각형 목록, 재생 = 칸 순서대로 카메라 pan/zoom + 그 칸의 대사, 타임라인 = 칸 순서와 `hold`(별도 트랙/키프레임 편집기 없음), fx 프리셋 enum(shake/flash), 음성/BGM/효과음 = 키. Story Editor v2 완료(§22) ~~③ 저작도구 공개 형태(Project Store가 로컬 JSON인지 Supabase인지)~~ → **결정(2026-09-12): 로컬 우선(v1)** — Published = 레포 JSON(git), Draft = serve.py `_draft/` 파일 + 자동 저장(`docs/superpowers/specs/2026-09-12-project-store-draft-design.md`); Supabase는 같은 4 엔드포인트(`draft-list/save/delete`·`publish-stage`)를 어댑터로 교체하는 자리 — 계정/권한이 필요해질 때 ④ 리깅 런타임(자체 컷아웃 확장 vs Spine 도입 — Presentation 데이터 계약은 어느 쪽이든 교체 가능하게) ⑤ GPL 부품(RPGAtlas 등) 코드 차용 여부 — 원하는 엔진 공개 정책과 라이선스 공개 범위를 먼저 결정(벤치마크 §13).
