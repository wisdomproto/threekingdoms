...
2. **② 등급계수 성장 이식 + 밸런스 재보정(§11) = W3** — 현 combat.ts는 `LV_GROWTH=3` 단일계수(잠정). 확보한 등급표를 `unitClasses.json`에 5스탯 등급 필드로 싣고, 성장을 원작 구간표(증분형) 룩업으로 교체. ⚠️ 모델이 재계산형→증분형으로 바뀌므로 사수관 등 전투 수치 재튜닝(sim 수백판 검증) 동반.
