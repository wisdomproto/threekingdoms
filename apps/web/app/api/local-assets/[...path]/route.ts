...
export async function GET(_request: Request, context: { params: Promise<{ path: string[] }> }): Promise<Response> {
  ...
  if (!(["maps", "objects", "sprites", "fx"].includes(path[0] ?? "") || (path[0] === "audio" && path[1] === "voices") || (path[0] === "ui" && ["portraits", "items"].includes(path[1]??""))) || path.some(part => !part || part === "." || part === ".." || /[\\/]/.test(part))) return new Response(null, { status: 400 });
  ...
  try {
    ...
    const type = ext === "mp3" ? "audio/mpeg" : ext === "webp" ? "image/webp" : ext === "png" ? "image/png" : ext === "json" ? "application/json" : "application/octet-stream";
