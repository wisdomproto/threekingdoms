...
export class UnitLayer extends Container {
  ...
  private createView(u: {
    ...
    const authored = this.ctx.stage.units.find(unit => unit.commanderId === u.id)
      ?? this.ctx.stage.reinforcements?.flatMap(group => group.units).find(unit => unit.commanderId === u.id);
    const facing = authored?.facing ?? (u.x < (this.ctx.map.width - 1) / 2 ? "right" : "left");
    view.setFacing(facing === "right" ? 1 : -1);
