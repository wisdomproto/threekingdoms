...
export function PortraitBox({ name, compact = false }: { name: string; compact?: boolean }): React.ReactElement | null {
  ...
  return (
    <div
      style={{
        ...PORTRAIT_FRAME,
        borderWidth: compact ? "3px" : "15px 11px 15px 11px",
        width: compact ? 40 : 56,
...
function TraitTab({ unit }: { unit: UnitVM }): React.ReactElement {
  ...
  return (
    <div style={{ paddingTop: 2 }}>
      ...
      {unit.traitText ? (
        <div style={{ fontSize: 11, color: "#cdbd92", lineHeight: 1.5, marginBottom: unit.passiveText ? 3 : 8 }}>
          {unit.traitText}
        </div>
      ) : null}
