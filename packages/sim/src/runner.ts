...
/**
 * 1판 실행. 하위호환: 두번째 인자가 number면 seed(+선택 maxTurns), 객체면 RunOpts.
...
export function runBattle(stageId: string, seedOrOpts?: number | RunOpts, maxTurns?: number): RunResult {
  ...
  const offset = opts.levelOffset ?? 0;

  const base = stages[stageId];
  if (!base) throw new Error(`unknown stage: ${stageId}`);
  const stage = withLevelOffset(base, offset);
  ...
  const cap = opts.maxTurns ?? stage.turnLimit + 1;
