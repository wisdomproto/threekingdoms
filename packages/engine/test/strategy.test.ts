...
// 간옹(책사, 초열/화진) + 적 2기를 십자 AoE 안에 배치한 합성 스테이지
const stage: Stage = {
...
describe("책략 (strategy) 액션", () => {
  it("책사는 자신의 책략 목록을 가진다 + MP 보유", () => {
    ...
    expect(gameData.unitClasses["strategist"]!.strategies).toContain("화진");
    expect(gan.mp).toBeGreaterThanOrEqual(gameData.strategies["화진"]!.mp); // 시전 가능
  });

  it("getStrategyTargets: 사거리 안 + AoE에 적 있는 칸만", () => {
    const s = createBattle(ctx, 1);
    const tiles = getStrategyTargets(ctx, s, "간옹", "화진");
    expect(tiles.some((t) => t.x === 4 && t.y === 4)).toBe(true); // 적 위치
    expect(tiles.some((t) => t.x === 0 && t.y === 0)).toBe(false); // 빈 구석
  });

  it("화진 시전: 십자 AoE로 적 2기 피해 + MP 소비 + acted, 무반격", () => {
    const s = createBattle(ctx, 1);
    const gan0 = s.units.find((u) => u.id === "간옹")!;
    const hua0 = s.units.find((u) => u.id === "화웅")!;
    const yi0 = s.units.find((u) => u.id === "이숙")!;
    const expectHua = strategyDamage(gan0, hua0, gameData.strategies["화진"]!.power);

    const r = applyAction(ctx, s, { type: "strategy", unitId: "간옹", strategyId: "화진", target: { x: 4, y: 4 } });
    const gan1 = r.state.units.find((u) => u.id === "간옹")!;
    const hua1 = r.state.units.find((u) => u.id === "화웅")!;
    const yi1 = r.state.units.find((u) => u.id === "이숙")!;

    expect(gan1.mp).toBe(gan0.mp - gameData.strategies["화진"]!.mp); // MP 소비
  ...
  it("회복 책략은 회복량을 troopsHealed 이벤트로 서술한다 (자기서술 계약 — 드레인 정합)", () => {
    ...
    const s = createBattle(ctx, 1);
    // 간옹을 부상 상태로(troops 20) 만들고 자기 칸(2,4)에 소보급(회복·단일·사거리3·target ally) 자가 시전
    const wounded = { ...s, units: s.units.map((u) => (u.id === "간옹" ? { ...u, troops: 20 } : u)) };
    const before = wounded.units.find((u) => u.id === "간옹")!.troops;
    const r = applyAction(ctx, wounded, { type: "strategy", unitId: "간옹", strategyId: "소보급", target: { x: 2, y: 4 } });
  ...
  it("MP 부족이면 시전 불가 (throw)", () => {
    ...
    expect(() =>
      applyAction(ctx, drained, { type: "strategy", unitId: "간옹", strategyId: "화진", target: { x: 4, y: 4 } }),
    ).toThrow();
  });
});
